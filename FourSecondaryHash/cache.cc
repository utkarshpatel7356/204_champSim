...
void CACHE::reset_heat() {
    if(total_heat < 5000) return;
    total_heat = 0;
    for(int i = 0; i < 2048; i++) heat[i] = 0;
}

uint32_t CACHE::secondary_hash(uint64_t address) {
    return (uint32_t) (1999 - (address % 1999));
}
...
uint32_t CACHE::get_set_1(uint64_t address)
{
...
{
    return (uint32_t) (get_set_1(address) + secondary_hash(address)) % 2048; 
}
uint32_t CACHE::get_set_3(uint64_t address)
{
    return (uint32_t) (get_set_1(address) + 2*secondary_hash(address) + 1) % 2048; 
}
uint32_t CACHE::get_set_4(uint64_t address)
{
    return (uint32_t) (get_set_1(address) + 3*secondary_hash(address) + 4) % 2048; 
}
...
int CACHE::find(uint32_t set, PACKET *packet) {
    // hit
    for (uint32_t way=0; way<NUM_WAY; way++) {
        total_comparisons++;
...
{
    total_memory_searches++;
    uint32_t set = get_set_1(packet->address);
    ...
    if(cache_type == IS_LLC && way == -1) {
        ...
        if(way != -1) {heat[set2]++; total_heat++; return std::make_pair(set2, way);}
        set2 = get_set_3(packet->address);
        way = find(set2, packet);
        if(way != -1) {heat[set2]++; total_heat++; return std::make_pair(set2, way);}
        set2 = get_set_4(packet->address);
        way = find(set2, packet);
        if(way != -1) {heat[set2]++; total_heat++; return std::make_pair(set2, way);}
    }

    if(way != -1) {total_heat++; heat[set]++;}
