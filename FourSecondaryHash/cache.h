...
class CACHE : public MEMORY {
  public:
    ...
    // extra
    int heat[2048];
    int total_heat, total_memory_searches, total_comparisons;

    uint32_t get_set_1(uint64_t address);
    uint32_t get_set_2(uint64_t address);
    uint32_t get_set_3(uint64_t address);
    uint32_t get_set_4(uint64_t address);
    uint32_t secondary_hash(uint64_t address);

    std::pair<int,int> check_hit(PACKET *packet);
    int find(uint32_t set, PACKET *packet);
    ...
    ~CACHE() {
        if(cache_type == IS_LLC) {
            double overhead = total_comparisons / (double)total_memory_searches;
            cout << "LLC overhead: " << overhead << endl;
        }
        for (uint32_t i=0; i<NUM_SET; i++)
            delete[] block[i];
        delete[] block;
