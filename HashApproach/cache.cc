#define EVICTION_AFTER 5000
#define HOTNESS_REDIRECTION 512
#include <iostream>
#include <vector>
#include <algorithm>
#include <utility>
			...
			if((WQ.entry[index].fill_l1d) && (MSHR.entry[mshr_index].fill_l1d != 1))
			  {
                ...
                if (cache_type == IS_LLC) {
                  address_set_mapping[WQ.entry[index].address] = set;
...
uint32_t CACHE::get_set(uint64_t address)//UTK_ just the regular get_set function
{
...
uint32_t CACHE::get_set_redirect(uint64_t address)//gives the redirected set location
{
  ...
  if(cache_type == IS_LLC)
    {
      if (redirect_lookup.find(val) != redirect_lookup.end()) {
        return redirect_lookup[val];
      }
    }
	return val;
}
...
std::pair<int, int> CACHE::check_hit(PACKET *packet)
{
    ...
    if(address_set_mapping.find(packet->address) != address_set_mapping.end()) {
      set=address_set_mapping[packet->address];
      for (uint32_t way=0; way<NUM_WAY; way++) {
        if (block[set][way].valid && (block[set][way].tag == packet->address)) {
...
void CACHE::print_evictions() {
    ...
    for(int i = 0; i < 2048; i++) {
        if(temp_evictions[i] > 0) count++;
        classification.push_back(std::make_pair(i, temp_evictions[i]));
    }
    ...
    // }
    for(int i=0;i<HOTNESS_REDIRECTION;i++){
      redirect_lookup[classification[2047-i].first]=classification[i].first;
    }
    for(int i=0;i<2048;i++){
      temp_evictions[i]=0;
