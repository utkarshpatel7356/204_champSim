//UTK _ line 113 uint64_t _defined array access[2048] to count the number of accesses of a prticular set
...
class CACHE : public MEMORY {
  public:
    ...
    uint64_t sim_access[NUM_CPUS][NUM_TYPES],
             ...
             evictions_count,//UTK_record the total eviction count
             temp_evictions[2048],//UTK_to check the currently hot elements
             roi_hit[NUM_CPUS][NUM_TYPES],
             roi_miss[NUM_CPUS][NUM_TYPES];
             
    map<int,int> redirect_lookup;//UTK_to check where to redirect
    map<uint64_t,int> address_set_mapping;//UTK_maps redirected address to the remapped set
    ...
    void return_data(PACKET *packet),
         ...
         print_evictions();//UTK_for preinting the hot elements and remapping the sets

    uint32_t get_occupancy(uint8_t queue_type, uint64_t address),
             get_size(uint8_t queue_type, uint64_t address);
    
    std::pair<int, int> check_hit(PACKET *packet);//UME_to check the hit in the cache
    ...
    uint32_t get_set(uint64_t address),
             get_set_redirect(uint64_t address),
             get_way(uint64_t address, uint32_t set),
             find_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type),
             llc_find_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type),
             lru_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type);
    int check_map_get_set(PACKET *packet);//check if address present in map and return set accordingly
};
