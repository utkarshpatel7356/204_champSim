//if just doing redirection try to just put in some cold set and just store where that address currently  is
#define EVICTION_AFTER 1000
#define HOTNESS_REDIRECTION 512
#define LOCALITY 4
#define HEX 0xFFFFFFFC
#include <iostream>
#include <vector>
#include <algorithm>
#include <utility>
...
uint32_t CACHE::get_set_redirect(uint64_t address)//gives the redirected set location
{
  ...
  if(cache_type == IS_LLC)
  {
    ...
    val = val & HEX;
    eps[val]++;
    // uint32_t minVal=temp_evictions[val];
    // for(uint32_t i=0;i<4;i++){
    //   if(temp_evictions[val+i]<minVal){
    //     minVal=temp_evictions[val+i];
    //     ans=i;
    //   }
    // }
    // cout<< val <<endl;
    ans=eps[val]%LOCALITY;
  }
	return val+ans;
}
