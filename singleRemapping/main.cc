...
void print_set_stats( CACHE *cache){
	vector<pair<int,int>>v1;//first is the evictions count, second is the set number
    map<int,vector<int>> mp;//set number is key m1->eviction count, m2->type4 is hottest,m3->access count ,m4->type4 is hottest
    for(int i=0;i<2048;i++)
    {
         v1.push_back(make_pair(cache->eviction_overall[i],i));
    }
    ...
    {
        mp[v1[i].second].push_back(v1[i].first);
        mp[v1[i].second].push_back(1);
        i--;
    }
    vector<pair<int,int>>v2;//first is the evictions count, second is the set number
    for(int i=0;i<2048;i++)
    {
         v2.push_back(make_pair(cache->access_overall[i],i));
    }
    ...
    while(j--)
    {
        mp[v2[i].second].push_back(v2[i].first);
        mp[v2[i].second].push_back(4);
       i--;
    }
    j=512;
    while(j--)
    ...
    {
        mp[v2[i].second].push_back(v2[i].first);
        mp[v2[i].second].push_back(1);
        i--;
    }
    for(int i=0;i<2048;i++)
    {
       cout << "set :" << i << " ";
       cout << "evictions: " << mp[i][0] ;
       cout << " type:";
       if(mp[i][1]==1)
        {
            cout << "Very Cold" << "\t";
